...
player.onChat("bw.join", function () {
    ...
    if (arena.playersNow == arena.maxPlayers) {
        ...
        for (let i = 0; i < arena.players.length - 1; i++) {
            ...
            let target = mobs.playerByName(arena.players[i])
            let rand = Math.randomRange(0, arena.players.length - 1)
            mobs.teleportToPosition(target, arena.spawns[rand])
            //TODO
        }
...
blocks.onBlockBroken(blocks.blockByName("bed"), function () {

    if(arena.isGame) {

        for(let i = 0; i < arena.players.length - 1; i++) {
            let playername = player.name()
            if(arena.players[i] == playername) {
                //TODO Make teams somehow in this shit /:
            }
        }
